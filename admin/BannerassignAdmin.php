  ...
  <TR>
    ...
    <table  id='gut' width="100%" border="0" align="center" cellspacing="4" cellpadding="3">
		...
		<?php }
			...
			$today = date("y-m-d");
			$query_Contents1 = "SELECT banners.bannerimg 	,banners.bannercaption 	,banners.bannerlink, bannerpages.pageid,bannerpages.bannerid,bannerpages.menuid,bannerpages.submenuid ,bannerpages.eventid,bannerpages.bannercatid FROM banners inner join bannerpages on bannerpages.bannerid = banners.bannerid WHERE banners.bannerid=$Edit";
		...
		<form name="form1" action="log.php" method="post" enctype="multipart/form-data">
		      ...
		      <td><img src="../images/<?php echo $row_Contents1['bannerimg']; ?>"></td>
				...
				<tr>
                  ...
                  <td>
                <select name="bannercatid"  id="bannercatid" style="width:320px">
						<?php echo bannercategory($row_Contents1['bannercatid'])?>
                      </select></td>
                  </tr>
				  <tr>
                  <td><strong>Menu Page</strong></td>
                  <td>
                <select name="menuid[]" id="menuid" size="4" multiple style="width:320px">
						<?php echo menu($row_Contents1['menuid'])?>
                      </select></td>
                  </tr>
				<tr>
                  <td><strong>SubMenu Page</strong></td>
                  <td>
                <select name="submenuid[]" size="4" multiple id="submenuid" style="width:320px">
						<?php echo submenu($row_Contents1['submenuid'])?>
                      </select></td>
                  </tr>
				  <tr>
                  <td><strong>Event Page</strong></td>
                  <td>
                <select name="eventid[]" size="4" multiple id="eventid" style="width:320px">
						<?php echo events($row_Contents1['eventid'])?>
                      </select></td>
                  </tr>
                <tr>
                  <td>&nbsp;</td>
                  <td><input type="submit" name="Submit" value="Edit">
                    <input type="hidden" name="bannerid" value="<?php echo $Edit;?>">
                    <input type="hidden" name="formname" value="BannerAssign"></td>
                  </tr>
